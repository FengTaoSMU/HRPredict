...
## create result score and class data frame
...
library(e1071)
model_result_get <- function(svm_model, input_word, cutoff){
  pred <- predict(svm_model, input_word, decision.values = TRUE)
  result <- data.frame(ID = rownames(input_word), distance = rep(0, nrow(input_word)), class = rep("No", nrow(input_word)))
...
model_predict <- function(input_word, model_dir, cutoff_list, result_class){
  for (rown in 1:nrow(cutoff_list)) {
    ...
    s_result <- model_result_get(s_model, input_word, cutoff_list[rown, 2])
    result_class[, rown] <- s_result$class
    colnames(result_class)[rown] <- unit_choose
    rm(rown, unit_choose, unit_dir, s_model, s_result)
  }
  result_class$ID <- rownames(input_word)
  result_class <- result_class[, c(ncol(result_class), 1:(ncol(result_class)-1)) ]
  return(result_class)
}

## load R package and predict
HR_Family <- model_predict(input_word, model_f_dir, input_family, result_family)
HR_Genus <- model_predict(input_word, model_g_dir, input_genus, result_genus)
HR_Species <- model_predict(input_word, model_s_dir, input_species, result_species)
